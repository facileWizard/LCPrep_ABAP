...
read table gt_hashed assigning field-symbol(<fs_hashed>) with table key key = gv_key.
if sy-subrc eq 0.
<fs_hashed>-value = gv_value.
else.
insert value #( key = gv_key value = gv_value ) into table gt_hashed.
endif.
endmethod.
...
read table gt_hashed into data(wa_hashed) with table key key = gv_key.
if sy-subrc eq 0.
